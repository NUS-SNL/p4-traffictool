...
function p4_proto.dissector(buffer,pinfo,tree)
	...
	local subtree = tree:add(p4_proto,buffer(),'P4_TCP Protocol Data')
		...
		subtree:add(buffer(12,1), 'res (3 bits):' .. string.format('%X', tostring(buffer(12,1):bitfield(4,3))))
		subtree:add(buffer(12,2), 'ecn (3 bits):' .. string.format('%X', tostring(buffer(12,2):bitfield(7,3))))
		subtree:add(buffer(13,2), 'ctrl (6 bits):' .. string.format('%X', tostring(buffer(13,2):bitfield(2,6))))
